...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let file_id_mapping_service = Arc::new(
        IdMappingService::new(file_id_mapping_path)
            .await
    ...
    let id_mapping_optimizer = Arc::new(IdMappingOptimizer::new(base_id_mapping_service.clone()));
            ...
            match self.repo.get_file(file_id).await {
                Ok(_) => {
                    ...
                    let parent_folder: Option<String> = if path_components.len() > 1 {
                        ...
                        tracing::info!(
                            "Attempting to restore to parent folder from path: {}",
                            original_path
                        );
                ...
                Err(e) => {
                    ...
                    Err(
                        domain::repositories::file_repository::FileRepositoryError::NotFound(
                            file_id.to_string(),
                        ),
                    )
                }
    ...
    if let Err(e) = i18n_service
        .load_translations(domain::services::i18n_service::Locale::Spanish)
        .await
    {
    ...
    let auth_services = if config.features.enable_auth && db_pool_ref.is_some() {
        ...
        .await
        {
            Ok(services) => {
                tracing::info!(
                    "Authentication services initialized successfully with folder service"
                );
                Some(services)
            }
            Err(e) => {
    ...
    let storage_mediator_stub =
        Arc::new(application::services::storage_mediator::FileSystemStorageMediator::new_stub());
    let metadata_manager = Arc::new(infrastructure::repositories::FileMetadataManager::default());
    let path_resolver_stub =
        Arc::new(infrastructure::repositories::FilePathResolver::default_stub());
    ...
    let favorites_service: Option<Arc<dyn application::ports::favorites_ports::FavoritesUseCase>> =
        if let Some(pool) = db_pool_ref {
            ...
            let favorites_service = Arc::new(FavoritesService::new(pool.clone()));
    ...
    if let Ok(count) = metadata_cache
        .preload_directory(&storage_path, true, 1)
        .await
    {
