...
impl FolderFsRepository {
    ...
    pub async fn get_mapped_folder_path(&self, folder_id: &str) -> FolderRepositoryResult<String> {
        let storage_path = self
            ...
            .await
            .map_err(|e| {
                FolderRepositoryError::MappingError(format!("Failed to get folder path: {}", e))
            })?;
    ...
    pub async fn update_mapped_folder_path(
        ...
        self.id_mapping_service
            ...
            .await
            .map_err(|e| {
                FolderRepositoryError::MappingError(format!("Failed to update folder path: {}", e))
            })
    }
    ...
    pub async fn remove_mapped_folder_id(&self, folder_id: &str) -> FolderRepositoryResult<()> {
        self.id_mapping_service
            ...
            .await
            .map_err(|e| {
                FolderRepositoryError::MappingError(format!("Failed to remove folder ID: {}", e))
            })
    }
    ...
    fn from(err: IdMappingError) -> Self {
        match err {
            ...
            IdMappingError::Timeout(msg) => {
                FolderRepositoryError::Other(format!("Timeout: {}", msg))
            }
    ...
    fn from(err: FolderRepositoryError) -> Self {
        match err {
            ...
            FolderRepositoryError::IoError(e) => {
                DomainError::internal_error("Folder", format!("IO error: {}", e)).with_source(e)
            }
...
impl FolderStoragePort for FolderFsRepository {
    ...
    async fn move_folder(
        ...
        FolderRepository::move_folder(self, id, new_parent_id)
            .await
            .map_err(DomainError::from)
    }

    async fn delete_folder(&self, id: &str) -> Result<(), DomainError> {
        FolderRepository::delete_folder(self, id)
            .await
            .map_err(DomainError::from)
    }

    async fn folder_exists(&self, storage_path: &StoragePath) -> Result<bool, DomainError> {
        FolderRepository::folder_exists_at_storage_path(self, storage_path)
            .await
            .map_err(DomainError::from)
    }

    async fn get_folder_path(&self, id: &str) -> Result<StoragePath, DomainError> {
        FolderRepository::get_folder_storage_path(self, id)
            .await
            .map_err(DomainError::from)
    }
...
impl FolderRepository for FolderFsRepository {
    ...
    async fn create_folder(
        ...
        let parent_storage_path = match &parent_id {
            Some(id) => match self.get_folder_storage_path(id).await {
                Ok(path) => {
                    tracing::info!(
        ...
        if self
            .folder_exists_at_storage_path(&folder_storage_path)
            .await?
        {
    ...
    async fn list_folders(&self, parent_id: Option<&str>) -> FolderRepositoryResult<Vec<Folder>> {
        ...
        let read_dir_result = match timeout(read_dir_timeout, fs::read_dir(&abs_parent_path)).await
        {
            ...
            Err(_) => {
                return Err(FolderRepositoryError::Other(format!(
                    "Timeout reading directory: {}",
                    abs_parent_path.display()
                )));
            }
        };
    ...
    async fn rename_folder(&self, id: &str, new_name: String) -> FolderRepositoryResult<Folder> {
        ...
        if self
            .folder_exists_at_storage_path(renamed_folder.storage_path())
            .await?
        {
            return Err(FolderRepositoryError::AlreadyExists(
    ...
    async fn move_folder(
        ...
        let target_parent_storage_path = match new_parent_id {
            Some(parent_id) => match self.get_folder_storage_path(parent_id).await {
                Ok(path) => Some(path),
                Err(e) => {
                    return Err(FolderRepositoryError::Other(format!(
        ...
        // Check if target already exists
        if self
            .folder_exists_at_storage_path(moved_folder.storage_path())
            .await?
        {
            return Err(FolderRepositoryError::AlreadyExists(format!(
        ...
        tracing::info!(
            "Folder moved successfully from {:?} to {:?}",
            old_abs_path,
            new_abs_path
        );
    ...
    async fn delete_folder(&self, id: &str) -> FolderRepositoryResult<()> {
        ...
        let delete_task = tokio::spawn(async move {
            ...
            if entry_count > 1000 {
                ...
                tracing::debug!(
                    "Using async removal for folder with {} entries",
                    entry_count
                );
        ...
        match delete_result {
            Ok(task_result) => match task_result {
                ...
                Err(join_err) => {
                    return Err(FolderRepositoryError::Other(format!(
                        "Task panicked during folder deletion: {}",
                        join_err
                    )));
                }
            },
