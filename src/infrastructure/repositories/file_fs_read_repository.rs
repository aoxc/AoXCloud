...
use crate::infrastructure::repositories::file_metadata_manager::{
    FileMetadataManager, MetadataError,
};
...
impl FileFsReadRepository {
    ...
    async fn create_file_entity(
        ...
        if let (Some(created), Some(modified)) = (created_at, modified_at) {
            ...
            .map_err(|e| {
                crate::domain::repositories::file_repository::FileRepositoryError::Other(
                    e.to_string(),
                )
            })
        } else {
            File::new(id, name, storage_path, size, mime_type, folder_id).map_err(|e| {
                crate::domain::repositories::file_repository::FileRepositoryError::Other(
                    e.to_string(),
                )
            })
    ...
    async fn get_file_by_id(&self, id: &str) -> FileRepositoryResult<File> {
        ...
        {
            return Err(
                crate::domain::repositories::file_repository::FileRepositoryError::NotFound(
        ...
        let (size, created_at, modified_at) = self
            .metadata_manager
            .get_file_metadata(&abs_path)
            .await
            .map_err(|e| match e {
                MetadataError::IoError(io_err) => {
                    crate::domain::repositories::file_repository::FileRepositoryError::IoError(
                        io_err,
                    )
                }
                MetadataError::Timeout(msg) => {
                    crate::domain::repositories::file_repository::FileRepositoryError::Timeout(msg)
                }
                MetadataError::Unavailable(msg) => {
                    crate::domain::repositories::file_repository::FileRepositoryError::NotFound(msg)
                }
            })?;
        ...
        let name = match storage_path.file_name() {
            ...
            None => {
                return Err(
                    crate::domain::repositories::file_repository::FileRepositoryError::InvalidPath(
...
impl FileReadPort for FileFsReadRepository {
    async fn get_file(&self, id: &str) -> Result<File, DomainError> {
        self.get_file_by_id(id).await.map_err(|e| match e {
            crate::domain::repositories::file_repository::FileRepositoryError::NotFound(msg) => {
                DomainError::not_found("File", msg)
            }
            crate::domain::repositories::file_repository::FileRepositoryError::IoError(io_err) => {
                DomainError::internal_error("File", io_err.to_string())
            }
            crate::domain::repositories::file_repository::FileRepositoryError::Timeout(msg) => {
                DomainError::internal_error("File", msg)
            }
    ...
    async fn get_file_content(&self, id: &str) -> Result<Vec<u8>, DomainError> {
        ...
        let file = self.get_file_by_id(id).await.map_err(|e| match e {
            crate::domain::repositories::file_repository::FileRepositoryError::NotFound(msg) => {
                DomainError::not_found("File", msg)
            }
            crate::domain::repositories::file_repository::FileRepositoryError::IoError(io_err) => {
    ...
    async fn get_file_stream(
        ...
        Err(DomainError::internal_error(
            "File stream",
            "Stream functionality not yet implemented",
        ))
    }
}
