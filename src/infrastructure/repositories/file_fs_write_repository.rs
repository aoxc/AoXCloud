...
impl FileFsWriteRepository {
    ...
    async fn ensure_parent_directory(&self, abs_path: &PathBuf) -> FileRepositoryResult<()> {
        if let Some(parent) = abs_path.parent() {
            ...
            .await
            .map_err(|_| {
                crate::domain::repositories::file_repository::FileRepositoryError::Timeout(format!(
...
impl FileWritePort for FileFsWriteRepository {
    async fn save_file(
        ...
        let storage_path = match &folder_id {
            Some(folder_id) => StoragePath::from_string(&format!("/{}/{}", folder_id, name)),
            None => StoragePath::from_string(&format!("/{}", name)),
        };
        ...
        self.metadata_manager
            ...
            .map_err(|e| match e {
                MetadataError::IoError(e) => {
                    DomainError::internal_error("File metadata", e.to_string())
                }
    ...
    async fn move_file(
        ...
        Err(DomainError::internal_error(
            "File move",
            "Move functionality not yet implemented",
        ))
    }
