...
impl BatchOperationService {
    ...
    pub fn default(file_service: Arc<FileService>, folder_service: Arc<FolderService>) -> Self {
    ...
    pub async fn copy_files(
        ...
        let operations = file_ids.into_iter().map(|file_id| {
            ...
            async move {
                ...
                let copy_result = file_service
                    .move_file(&file_id, target_folder.clone())
                    .await;
    ...
    pub async fn move_files(
        ...
        info!(
            "Iniciando movimiento en lote de {} archivos",
            file_ids.len()
        );
        let start_time = std::time::Instant::now();
    ...
    pub async fn delete_files(
        ...
        info!(
            "Iniciando eliminación en lote de {} archivos",
            file_ids.len()
        );
        let start_time = std::time::Instant::now();
    ...
    pub async fn delete_folders(
        ...
        info!(
            "Iniciando eliminación en lote de {} carpetas",
            folder_ids.len()
        );
        let start_time = std::time::Instant::now();
    ...
    {
        info!(
            "Iniciando operación genérica en lote con {} items",
            items.len()
        );
        let start_time = std::time::Instant::now();
...
mod tests {
    ...
    async fn test_batch_delete_files() {
        ...
        file_service.expect_delete_file().times(3).returning(|id| {
            if id == "error-id" {
        ...
        let file_ids = vec!["id1".to_string(), "id2".to_string(), "error-id".to_string()];
