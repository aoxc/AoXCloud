...
impl From<FileRepositoryError> for FileServiceError {
    fn from(err: FileRepositoryError) -> Self {
        match err {
            ...
            FileRepositoryError::Timeout(msg) => {
                FileServiceError::AccessError(format!("Operation timed out: {}", msg))
            }
    ...
    fn from(err: DomainError) -> Self {
        match err.kind {
            crate::common::errors::ErrorKind::NotFound => {
                FileServiceError::NotFound(err.to_string())
            }
            crate::common::errors::ErrorKind::AlreadyExists => {
                FileServiceError::Conflict(err.to_string())
            }
            crate::common::errors::ErrorKind::InvalidInput => {
                FileServiceError::InvalidPath(err.to_string())
            }
            crate::common::errors::ErrorKind::AccessDenied => {
                FileServiceError::AccessError(err.to_string())
            }
            _ => FileServiceError::InternalError(err.to_string()),
        }
    }
...
impl From<FileServiceError> for DomainError {
    fn from(err: FileServiceError) -> Self {
        match err {
            ...
            FileServiceError::InvalidPath(path) => {
...
impl FileService {
    ...
    pub async fn get_file_by_path(&self, path: &str) -> FileServiceResult<FileDto> {
        ...
        Err(FileServiceError::NotFound(format!(
...
impl FileUseCase for FileService {
    async fn upload_file(
        ...
        FileService::upload_file_from_bytes(self, name, folder_id, content_type, content)
            .await
            .map_err(DomainError::from)
    }

    async fn get_file(&self, id: &str) -> Result<FileDto, DomainError> {
        FileService::get_file(self, id)
            .await
            .map_err(DomainError::from)
    }

    async fn get_file_by_path(&self, path: &str) -> Result<FileDto, DomainError> {
        FileService::get_file_by_path(self, path)
            .await
            .map_err(DomainError::from)
    }
    ...
    async fn create_file(
        ...
        FileService::create_file(self, parent_path, filename, content, content_type)
            .await
            .map_err(DomainError::from)
    }

    async fn update_file(&self, path: &str, content: &[u8]) -> Result<(), DomainError> {
        FileService::update_file(self, path, content)
            .await
            .map_err(DomainError::from)
    }

    async fn list_files(&self, folder_id: Option<&str>) -> Result<Vec<FileDto>, DomainError> {
        FileService::list_files(self, folder_id)
            .await
            .map_err(DomainError::from)
    }

    async fn delete_file(&self, id: &str) -> Result<(), DomainError> {
        FileService::delete_file(self, id)
            .await
            .map_err(DomainError::from)
    }

    async fn get_file_content(&self, id: &str) -> Result<Vec<u8>, DomainError> {
        FileService::get_file_content(self, id)
            .await
            .map_err(DomainError::from)
    }
    ...
    async fn get_file_stream(
        ...
        FileService::get_file_stream(self, id)
            .await
            .map_err(DomainError::from)
    }
    ...
    async fn move_file(
        ...
        FileService::move_file(self, file_id, folder_id)
            .await
            .map_err(DomainError::from)
    }
}
