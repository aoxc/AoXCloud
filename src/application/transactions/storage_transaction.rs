...
impl StorageTransaction {
        ...
        for (i, op) in operations.into_iter().enumerate() {
            match op().await {
                Ok(()) => {
                    ...
                    tracing::trace!(
                        "Operación {} completada en transacción: {}",
                        i,
                        transaction_name
                    );
                }
                Err(e) => {
                    tracing::error!(
                        "Error en operación {} de transacción {}: {}",
                        i,
                        transaction_name,
                        e
                    );
    ...
    async fn rollback(mut self, completed_ops: Vec<usize>) -> Result<(), DomainError> {
            ...
            if i < rollbacks.len() {
                ...
                if let Some(rb) = rollbacks.get_mut(i) {
                    ...
                    if let Err(e) = rollback().await {
                        tracing::error!(
                            "Error en rollback de operación {} en transacción {}: {}",
                            i,
                            self.name,
                            e
                        );
        ...
        if !rollback_errors.is_empty() {
            tracing::error!(
                "Errores durante rollback de transacción {}: {} errores",
                self.name,
                rollback_errors.len()
            );
